...
export default async function StudioPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
    ...
    return (
      <main className="studio-gate">
        <section>
          ...
          <div className="studio-login-state"><i />目前未登入</div>
          {error && <div className="studio-login-error" role="alert">{error}</div>}
          <StudioLogin clientId={config.clientId} loginUri={loginUri} />
  ...
  return <main className="studio-page"><StudioClient initialPosts={posts} email={session.email} sessionExpiresAt={session.expiresAt} /></main>;
}
